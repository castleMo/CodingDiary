...
- 시스템 호출과 API, SDK
    ...
    3. SDK
        - 프로그램 개발자를 위한 API, 코드 편집기, 에뮬레이터 같은 각종 개발용 응용 프로그램까지 하나로 묶어서 배포하는 개발 툴이다
- 커널의 종류
    1. 단일형 구조 커널
...
- 메모리 보호(memory protection)란
    - 메모리 보호는 프로그램이 잘못된 메모리 영역에 접근하여 오류가 발생하는 것을 막는 기능이다
    - 메모리 보호는 하드웨어와 소프트웨어로 구현할 수 있다
- 메모리 보호가 필요한 이유
    - 현대의 운영체제는 시분할 기법을 사용하여 여러 프로그램을 동시에 실행하므로 사용자 영역이 여러 개의 작업 공간으로 나뉘어 있다
    - 이때 프로그램이 잘못된 메모리 영역에 접근하면 다른 프로그램에 영향을 미칠 수 있기 때문에 메모리 보호가 필요하다
- CPU는 현재 진행 중인 작업의 메모리 시작 주소를 경계 레지스터(bound register)에 저장하고 현재 진행 중인 작업이 차지하고 있는 메모리의 크기, 즉 마지막 주소까지의 차이를 한계 레지스터(limit
  register)에 저장한 후 작업이 진행되는 동안 이 두 제리스터의 주소 범위를 벗어나는지 하드웨어 측면에서 점검함으로써 메모리를 보호한다
- 만약 두레지스터의 값을 벗어나나다면 메모리 오류와 관련된 인터럽트가 발생한다

#### 부팅

응용 프로그램은 운영체제가 메모리에 올려서 실행하는데 운영체제는 누가 메모리에 올려서 실행할까?

- 부팅(booting)이란
    - 부팅은 컴퓨터를 켜서 운영체제를 사용할 수 있는 상태로 만드는 과정을 말한다
    - 부팅은 전원을 켜면서 컴퓨터가 자가 진단을 하고 운영체제를 메모리에 올리는 과정으로 이루어진다
- 부팅 과정
    1. 전원을 켜면서 컴퓨터가 자가 진단을 한다
    2. 자가 진단이 완료되면 운영체제를 메모리에 올린다
    3. 운영체제가 메모리에 올라오면서 컴퓨터가 사용할 수 있는 상태가 된다
```mermaid
graph LR
    바이오스["바이오스(롬)"] --> 하드웨어_점검[하드웨어 점검]

    하드웨어_점검 --> 메모리에_부트스트랩_코드를_올려서_실행[메모리에 부트스트랩 코드를 올려서 실행]
    마스터_부트_레코드[마스터 부트 레코드] --> 메모리에_부트스트랩_코드를_올려서_실행[메모리에 부트스트랩 코드를 올려서 실행]

    메모리에_부트스트랩_코드를_올려서_실행 --> 부트스트랩_코드가_운영체제_프로세스_실행[부트스트랩 코드가 운영체제 프로세스 실행]
    운영체제 --> 부트스트랩_코드가_운영체제_프로세스_실행
```
...
- 버퍼(buffer)란
  - 두 장치 사이의 속도 차이를 완화하는 역할을 한다 
  - 버퍼는 데이터를 한 곳에서 다른 한 곳으로 전송하는 동안 일시적으로 그 데이터를 보관하는 임시 메모리 영역이다
- 버퍼는 하드웨어에서만 사용되는 개념은 아니다
  - 버퍼가 소프트웨어에서 사용되는 대표적인 예는 동영상 스트리밍이다

#### 캐시

- 캐시(cache)란
  - 캐시는 CPU와 메인메모리 사이의 속도 차이를 완화하는 역할을 한다 (임시 저장소이다)
  - 캐시는 CPU가 자주 사용하는 데이터를 미리 복사해 놓는 임시 메모리 영역이다
  - 캐시는 CPU와 메인메모리 사이에 위치하며 CPU가 데이터를 요청하면 캐시에서 먼저 찾아보고 있으면 캐시에서 데이터를 가져온다
  - 캐시는 CPU가 자주 사용하는 데이터를 미리 복사해 놓기 때문에 CPU가 데이터를 요청할 때마다 메인메모리에 접근할 필요가 없어서 CPU의 성능을 향상시킨다
- 캐시의 종류
  - 캐시는 CPU 내부에 있는 L1, L2, L3 캐시와 CPU와 메인메모리 사이에 있는 L4 캐시로 구분된다
  - L1, L2, L3 캐시는 CPU 내부에 있기 때문에 속도가 빠르고 L4 캐시는 CPU와 메인메모리 사이에 있기 때문에 속도가 느리다
- 캐시에서 원하는 데이터를 찾았을 떄 캐시 히트라고 하고 원하는 데이터를 찾지 못했을 때 캐시 미스라고 한다
- 캐시 히트가 되는 비율을 캐시 적중률 이라고 하며 일반적인 컴퓨터의 캐시 적중률은 약 90%다
- 캐시 데이터 변경 방법
  - 즉시 쓰기(write through)
    - 캐시에 있는 데이터가 변경되면 이를 즉시 메모리에 만영하는 방식이다
    - 메모리와의 빈번한 데이터 전송으로 성능이 느려진다는 것이 단점이다
    - 하지만 메모리의 최신값이 항상 유지되기 때문에 급작스러운 정전에도 데이터를 잃어버리지 않는다
  - 지연 쓰기(write back)
    - 캐시에 있는 데이터가 변경되면 그 내용을 모아서 주기적으로 반영하는 방식으로 카피팩이라고도 한다
    - 메모리와의 데이터 전송 횟수가 줄어들어 시스템 성능을 향상할 수는 있으나 메모리와 캐시된 데이터 사이의 불일치가 발생할 수 있다는 것이 단점이다

#### 저장장치의 계층 구조

![storage-hierarchy.png](images/part1/storage-hierarchy.png)

#### 인터럽트

- 인터럽트(interrupt)란
  - 인터럽트는 CPU가 프로그램을 실행하고 있을 때 예기치 않은 상황이 발생하여 처리가 필요할 때 CPU에 알려 예외 상황을 처리하는 기술이다
- 인터럽트의 종류
  - 하드웨어 인터럽트
    - 하드웨어 인터럽트는 CPU가 프로그램을 실행하고 있을 때 입출력장치 등의 하드웨어 장치가 CPU에게 처리가 필요한 상황이 발생했음을 알려 예외 상황을 처리하는 기술이다
  - 소프트웨어 인터럽트
    - 소프트웨어 인터럽트는 CPU가 프로그램을 실행하고 있을 때 프로그램 내부에서 예외 상황이 발생했음을 알려 예외 상황을 처리하는 기술이다
- 인터럽트의 처리 과정
  1. CPU는 프로그램을 실행하고 있을 때 인터럽트가 발생했는지 주기적으로 확인한다
  2. 인터럽트가 발생했다면 현재 실행 중인 프로그램을 멈추고 인터럽트 처리 루틴을 실행한다
  3. 인터럽트 처리 루틴이 끝나면 이전에 실행하던 프로그램을 다시 실행한다

#### 직접 메모리 접근

- 직접 메모리 접근(DMA, Direct Memory Access)이란
  - 직접 메모리 접근은 CPU의 개입 없이 주변장치가 메모리에 데이터를 직접 읽고 쓰는 기술이다
  - 직접 메모리 접근은 주로 하드디스크나 네트워크 카드 등의 대용량 데이터를 처리할 때 사용된다

#### 메모리 맵 입출력

- 메모리 맵 입출력(memory-mapped I/O)이란
  - 메모리 맵 입출력은 입출력장치를 메모리에 접속시켜 입출력장치를 메모리처럼 사용하는 기술이다
